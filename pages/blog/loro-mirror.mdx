...
image: "https://i.ibb.co/bRM0JwMC/image.png"
---
...
**Loro** is a CRDT library for local‑first apps. It supports rich containers—`Text`, `Map`, `List`/`MovableList`, `Tree`—with versioning, time‑travel, and compact updates/snapshots.

Apps still need glue code to map between CRDT documents and UI state. CRDTs ensure documents converge; they do not maintain your application state.
...
For an update, if **k** items change and each changed item affects **m** of its immediate fields, time complexity is **≈ O(k·m)**. _(k = number of changed items; m = average number of changed immediate fields per changed item.)_ This time complexity is quite similar to the time complexity of React’s render function.

## Why this exists

Without Mirror, projects that uses Loro need to:

1. Map CRDT containers to UI data structures.
...
- **Typed and framework‑agnostic.** Works in plain TypeScript or React (via `loro-mirror-react`).
- **Fine‑grained diffs.** Generates ops such as item moves in `MovableList` and character deltas in `Text`.
- **Predictable subscriptions.** Emits focused updates for stable renders.
- **Sync‑friendly.** Uses Loro updates/snapshots over any transport (HTTP, WebSocket, P2P).
