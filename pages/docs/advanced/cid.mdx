...
When you get a container through the document API like `doc.getText("text")` or from the return value of `map.setContainer("key", new LoroText())`, the container is **attached**:
...
Note: When using `setContainer()`, the container passed as parameter remains detached, but the method returns a new attached container.
...
// setContainer returns an attached container
const detachedText = new LoroText();
const attachedText = attachedMap.setContainer("text", detachedText);
// detachedText remains detached
// attachedText is the attached version with proper ContainerID
```
