...
`node.data` is a Map. If the child container under `node.data` may be lazily
created by multiple peers at the same key, use a mergeable child such as
`node.data.ensureMergeableList("list")` instead of `setContainer`.
