...
- Stream updates over WebSocket: see [`subscribeLocalUpdates`](#LoroDoc.subscribeLocalUpdates) and [`import`](#LoroDoc.import) under [Synchronization](#synchronization).
- Redact leaked secrets from history: see [`redactJsonUpdates`](#LoroDoc.redactJsonUpdates) in [Synchronization](#synchronization).
- Undo/redo only my edits: see [Undo/Redo](#undoredo) and `UndoManager`.
- Fork a branch and merge later: see [Versioning](#versioning) and [`fork`](#LoroDoc.fork)/[`forkAt`](#LoroDoc.forkAt) + [`export`](#LoroDoc.export)/[`import`](#LoroDoc.import).
- Time travel and view older versions: see [Versioning](#versioning) and [`checkout`](#LoroDoc.checkout)/[`checkoutToLatest`](#LoroDoc.checkoutToLatest).
- Fast load with current state only: export a [shallow snapshot](#synchronization) and import it.
- Track peer metadata when first seen: use [`subscribeFirstCommitFromPeer`](#LoroDoc.subscribeFirstCommitFromPeer) in [Events & Transactions](#events--transactions).
- Tag implicit commits (import/export/checkout): use [`subscribePreCommit`](#LoroDoc.subscribePreCommit) in [Events & Transactions](#events--transactions).
- Find containers changed in a change range: see [`getChangedContainersIn`](#LoroDoc.getChangedContainersIn) in [LoroDoc](#lorodoc).
- Diff two versions: see [`diff`](#LoroDoc.diff) in [LoroDoc](#lorodoc) and [Versioning](#versioning).
