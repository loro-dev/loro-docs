...
const snapshot: Uint8Array = // ... from storage or network
...
const updates: Uint8Array = // ... from network or storage
...
- `insert()` and `delete()` use UTF-16 code unit indices (same as JavaScript string indices)
- `length` returns the UTF-16 code unit count (same as JavaScript `string.length`)
- Characters outside the Basic Multilingual Plane (e.g., emoji 😀) count as 2 units
- Use `insertUtf8()` and `deleteUtf8()` for UTF-8 byte-based operations if needed
...
console.log(text.toString()[6]); // Invalid - splits the emoji
text.delete(6, 2);               // Deletes the entire emoji
```
...
**Warning:** May return invalid results for positions inside multi-unit characters (e.g., emoji).
...
text.insert(0, "😀");
const invalid = text.charAt(0); // First half of emoji (invalid)
const valid = text.charAt(2);   // "H" from "Hello"
```
</Indent>
...
**Warning:** Be careful not to slice in the middle of multi-unit characters.
...
**Note:** Emoji and other characters outside the Basic Multilingual Plane count as 2 units.
