...
- Stream updates over WebSocket: see `subscribeLocalUpdates` and `import` under [Synchronization](#synchronization).
- Redact leaked secrets from history: see `redactJsonUpdates` in [Synchronization](#synchronization).
- Undo/redo only my edits: see [Undo/Redo](#undoredo) and `UndoManager`.
- Fork a branch and merge later: see [Versioning](#versioning) and `fork`/`forkAt` + `export`/`import`.
- Time travel and view older versions: see [Versioning](#versioning) and `checkout`/`checkoutToLatest`.
- Fast load with current state only: export a [shallow snapshot](#synchronization) and import it.
- Track peer metadata when first seen: use `subscribeFirstCommitFromPeer` in [Events & Transactions](#events--transactions).
- Tag implicit commits (import/export/checkout): use `subscribePreCommit` in [Events & Transactions](#events--transactions).
- Find containers changed in a change range: see `getChangedContainersIn` in [LoroDoc](#lorodoc).
- Diff two versions: see `diff` in [LoroDoc](#lorodoc) and [Versioning](#versioning).
...
- Configuration: [`setPeerId`](#method-setPeerId), [`setChangeMergeInterval`](#method-setChangeMergeInterval), [`configTextStyle`](#method-configTextStyle)
- Containers: [`getText`](#method-getText), [`getList`](#method-getList), [`getMap`](#method-getMap), [`getTree`](#method-getTree), [`getCounter`](#method-getCounter), [`getMovableList`](#method-getMovableList)
- Sync: [`export`](#method-export), [`import`](#method-import), [`importBatch`](#method-importBatch), [`exportJsonUpdates`](#method-exportJsonUpdates), [`importJsonUpdates`](#method-importJsonUpdates), metadata: [`decodeImportBlobMeta`](#method-decodeImportBlobMeta), [`redactJsonUpdates`](#method-redactJsonUpdates)
- Versioning: [`checkout`](#method-checkout), [`checkoutToLatest`](#method-checkoutToLatest), [`fork`](#method-fork), [`forkAt`](#method-forkAt), convert: [`frontiersToVV`](#method-frontiersToVV), [`vvToFrontiers`](#method-vvToFrontiers)
- Events & Transactions: [`subscribe`](#method-subscribe), [`subscribeLocalUpdates`](#method-subscribeLocalUpdates), [`subscribeFirstCommitFromPeer`](#method-subscribeFirstCommitFromPeer), [`commit`](#method-commit), helpers: [`setNextCommitMessage`](#method-setNextCommitMessage), [`setNextCommitOrigin`](#method-setNextCommitOrigin), [`setNextCommitTimestamp`](#method-setNextCommitTimestamp), [`setNextCommitOptions`](#method-setNextCommitOptions), [`clearNextCommitOptions`](#method-clearNextCommitOptions), [`subscribePreCommit`](#method-subscribePreCommit)
- Query/Introspection: [`toJSON`](#method-toJSON), [`getShallowValue`](#method-getShallowValue), [`getAllChanges`](#method-getAllChanges), [`getChangeAt`](#method-getChangeAt), [`getOpsInChange`](#method-getOpsInChange), [`getPendingTxnLength`](#method-getPendingTxnLength), [`changeCount`](#method-changeCount), [`opCount`](#method-opCount)
...
- Switch: `checkout`, `checkoutToLatest`
- Branch: `fork`, `forkAt`
- Convert: `frontiersToVV`, `vvToFrontiers`
- Inspect: `diff`, `revertTo`, `applyDiff`
...
- Subscribe: `subscribe`, `subscribeLocalUpdates`, `subscribeFirstCommitFromPeer`
- Commit: `commit` with `origin`, `message`, `timestamp`, and next-commit helpers
- Hooks: Pre-Commit Hook `subscribePreCommit`
...
- Export modes: `export({ mode: ... })` — snapshot, update, shallow-snapshot, updates-in-range
- Import variants: `import`, `importBatch`
- JSON tools: `exportJsonUpdates`, `importJsonUpdates`, `redactJsonUpdates`
- Metadata: `decodeImportBlobMeta`
