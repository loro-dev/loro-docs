...
- All text operations use UTF-16 indices (JavaScript native)
- Emoji/Unicode characters outside BMP count as 2 units
- Slicing in the middle of multi-unit characters corrupts them
- Use `insertUtf8()`/`deleteUtf8()` for UTF-8 systems

### Document Initialization
- Don't edit immediately after `new LoroDoc()` - causes divergence
- Initialize from the same snapshot across all peers
- Load WASM before using LoroDoc in browser environments

### Container Creation
- Concurrent child container creation at same key causes overwrites
- Use unique root containers: `doc.getMap("data." + userId)`
- Initialize all child containers upfront when possible

### Events & Transactions
- Events emit asynchronously after a microtask
- Import/export/checkout trigger automatic commits
- Loro transactions are NOT ACID - no rollback/isolation

### Version Control
- After `checkout()`, document enters read-only "detached" mode
- Frontiers can't determine complete operation sets without history
- Shallow snapshots limit sync to peers with newer versions

### Data Structure Choice
- Use Map for coordinates, not List (avoids [x, y_a, y_b] problem)
- Use MovableList for drag-and-drop, List for append-heavy ops
...
- **Sync between peers**: [`export`](#LoroDoc.export) with `mode: "update"` + [`import`](#LoroDoc.import) - Exchange incremental updates
...
### Performance & Storage
- **Shallow snapshots**: [`export`](#LoroDoc.export) with `mode: "shallow-snapshot"` - State without history
...
- **Detached editing**: [`setDetachedEditing`](#LoroDoc.setDetachedEditing) - Stage changes before committing
...
**⚠️ Critical: Document Initialization Pitfall**
```typescript
// ❌ WRONG - Don't edit immediately after creation
const doc = new LoroDoc();
doc.getText("title").insert(0, "Initial Title");
// Each peer creates different operations - causes divergence!

// ✅ CORRECT - Use the same snapshot for all peers
const initialSnapshot = createInitialDocument(); // Create once
const doc = LoroDoc.fromSnapshot(initialSnapshot); // Load on all peers
```

**📝 WASM Initialization Note:**
In Vite/browser environments, ensure WASM is loaded before DOMContentLoaded:
```typescript
import init from "loro-crdt/wasm";
await init(); // Must complete before using LoroDoc
```
...
**⚠️ Pitfall:** Avoid concurrent creation of child containers with the same key in maps. Instead of:
```ts
...
**⚠️ Important:** The document automatically commits pending operations before import. Operations in detached mode are recorded but not applied to document state until you call `attach()`.
...
<Indent>
**⚠️ Important:** In Loro 1.0, `version()`/`frontiers()` include pending (uncommitted) local operations. If you need a stable commit boundary, call `commit()` before reading versions.
...
**⚠️ Important:** Events are emitted asynchronously after a microtask. If you need to handle events immediately after `commit()`/`import()`/`checkout()`, you must await a microtask:
```ts
await new Promise(resolve => setTimeout(resolve, 0));
// Now events have been emitted
```

**📝 Note:** Multiple operations before a commit are batched into a single event. The Subscription object must be kept alive to avoid garbage collection. See [Event Handling](/docs/tutorial/event).
</Indent>
...
- Emoji and characters outside the Basic Multilingual Plane count as **2 units**
- Use `insertUtf8()` and `deleteUtf8()` for UTF-8 byte-based operations when integrating with UTF-8 systems

**⚠️ Common Pitfalls:**
1. **Emoji Splitting**: Slicing or deleting in the middle of multi-unit characters corrupts them
2. **Index Misalignment**: UTF-16 indices differ from visual character count
3. **Cursor Positions**: Cursor positions use UTF-16 indices and may become slow when pointing to deleted text
4. **Performance**: Cursor queries on deleted positions require history traversal - refresh cursors periodically
...
- Example: URLs should be strings in maps, not LoroText, to avoid merge conflicts
...
**⚠️ Warning:** Returns invalid results for positions inside multi-unit characters (e.g., emoji). The result will be a partial UTF-16 code unit, not a valid character.
...
**⚠️ Warning:** Slicing in the middle of multi-unit characters (emoji, certain Unicode) creates invalid strings. Always ensure your start/end positions align with character boundaries.
...
**⚠️ Pitfall:** Concurrent child container creation at the same key from multiple peers causes overwrites (different container IDs cannot auto-merge). 

**Best practices:**
1. Initialize all child containers upfront during parent creation
2. Use unique root containers per user: `doc.getMap("user." + userId)`
3. Avoid: `map.getOrCreateContainer(key, new LoroMap())` in concurrent contexts

See [Container initialization](/docs/tutorial/tips).
...
**⚠️ Pitfall:** Parallel container creation for the same key across peers causes overwrites.

**Safe pattern:**
```typescript
// Instead of shared child containers:
// doc.getMap("shared").getOrCreateContainer("data", new LoroMap())

// Use unique root containers:
doc.getMap(`shared.data.${userId}`)
```

See [Tips](/docs/tutorial/tips).
...
- **Deletion is a move operation**: Nodes are moved to a special "deleted" location, not removed
...
- **Don't disable fractional index** if you need siblings to be sorted
- **Network delays**: Can temporarily create invalid states (cycles) before being rejected
...
- **Use List** for: Append-heavy operations, chat messages, logs
- **Key difference**: MovableList handles concurrent moves better (no duplicates), List is faster for insertions
...
- **Sanitization**: You must distribute sanitized/redacted documents to all peers - original sensitive data persists on other clients
- **Shallow snapshots**: Peers can only sync if they have versions after the shallow snapshot point
- **Import order**: Loro handles out-of-order updates automatically via the ImportStatus
...
- **Clear after major operations**: Consider clearing undo stack after document resets or major imports
...
**⚠️ Important:** 
- EphemeralStore is a separate CRDT without history - data is NOT persisted
...
- Uses Last-Write-Wins, not operational transformation like main document
