...
import CustomerWall from "./CustomerWall";
...
export default function LandingPage(): JSX.Element {
  const { cta, sections, build } = useHomeTranslations();
      ...
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        <section className="px-5 md:px-15 mt-10 z-10 relative flex justify-center">
          <div className="relative group">
            <div className="absolute -inset-0.5 bg-gradient-to-r from-blue-500 to-green-400 rounded-lg blur opacity-60 group-hover:opacity-100 transition duration-300" />
            <a
        ...
        <section className="px-5 md:px-15 mt-10 z-10 relative">
          <h2 className="text-2xl not-italic font-semibold leading-[normal] bg-clip-text mb-4 bg-blue-green text-fill-transparent">
            {sections.syncHeading[0]}
            <br />
            {sections.syncHeading[1]}
          </h2>
          <div className="nextra-code relative mt-6 first:mt-0 max-w-[100%] w-fit px-6 py-6 nextra-focus _overflow-x-auto _subpixel-antialiased _text-[.9em] _bg-white dark:_bg-black _py-4 _ring-1 _ring-inset _ring-gray-300 dark:_ring-neutral-700 contrast-more:_ring-gray-900 contrast-more:dark:_ring-gray-50 contrast-more:_contrast-150 _rounded-md">
            <pre className="_bg-transparent _m-0 _p-0">
              <code className="language-ts twoslash">
                {CODE_SNIPPET_LINES.join("\n")}
              </code>
            </pre>
          </div>
        ...
        <section className="px-5 md:px-15 mt-24 z-10 relative">
          <h2 className="text-2xl not-italic font-semibold leading-[normal] bg-clip-text mb-6 bg-blue-green text-fill-transparent">
            {sections.whatYouCanBuild}
          </h2>

          <div className="opacity-80 mb-6">
            {sections.whatYouCanBuildDescription}
          </div>

          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {build.cards.map((card, index) => {
              const Icon = BUILD_ICONS[index] ?? FileText;

              return (
                <div
                  key={card.title}
                  className="group rounded-xl border border-white/10 bg-zinc-800/40 hover:bg-zinc-800/60 transition-colors p-5"
                >
                  <div className="flex items-center mb-3">
                    <Icon
                      className="inline w-7 h-7 mr-3 align-text-bottom"
                  ...
                  {card.links.length > 0 && (
                    <div className="mt-3 text-sm text-blue-400 hover:text-blue-300">
                      <span className="mr-1">→</span>
                      {card.links.map((link, linkIndex) => (
                        <Fragment key={link.href}>
                          {linkIndex > 0 && <span>{build.linkSeparator}</span>}
