...
export default function Footer(): JSX.Element {
  ...
  return (
    <footer className="mx-5 md:mx-15 py-8 flex flex-col sm:flex-row justify-between items-center gap-4 text-white/50">
      <div className="flex items-center gap-2">
        <span className="font-semibold text-white/70">Loro</span>
      ...
      <div className="flex items-center gap-1">
        {socialLinks.map((link) => (
          <a
            ...
            className={cn(
              "p-2.5 rounded-xl text-white/50 hover:text-white hover:bg-white/10",
              "transition-all duration-300 ease-out",
              "hover:scale-110 hover:-translate-y-0.5"
            )}
            onClick={() => {
              window.clarity?.("event", link.event);
            }}
          >
